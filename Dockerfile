...
######
# The dependency "grimbough/Rhdf5lib" can be deleted when R 3.5.0 will be available. As for now, mzR 2.13.8 needs Rhdf5lib > 1.1.4
######
RUN apt-get update && apt-get install -y --no-install-recommends libboost-dev libcurl4-openssl-dev libnetcdf-dev libssl-dev libssh2-1-dev libxml2-dev libxml2-utils icu-devtools netcdf-bin r-base r-base-dev && \
	...
	R -e 'source("https://bioconductor.org/biocLite.R"); install.packages("devtools"); library("devtools"); install_github("grimbough/Rhdf5lib"); install_github("sneumann/mzR"); install.packages("batch"); install.packages("rsm"); install.packages("optparse");' && \
